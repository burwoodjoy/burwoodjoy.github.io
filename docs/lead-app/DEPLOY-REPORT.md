...
## 8. 변경 이력 — 2026-08-19: 삭제 버튼 · 태국어 · 프리텐다드 (사용자 결정)

**1) 대시보드 삭제 버튼**

- 마이그레이션 `0003_thai_lang_and_delete.sql` 적용 — DELETE 정책(`public_delete_leads`) 개방. 무로그인 정책과 일관.
- 카드마다 삭제 버튼 추가. 실수 방지를 위해 **2단계 확인**: 1차 클릭 시 "정말 삭제"(붉은색)로 전환되고 4초 내 재클릭해야 실제 삭제. 검증: 테스트 행 삭제 성공, 실제 리드(#5)는 보존.

**2) 태국어(th) 추가 + 번역 검수**

- 언어 바에 ไทย 추가, 전체 키 태국어 번역. DB `lang` CHECK 제약에 'th' 추가(0003에 포함) — 누락 시 태국어 제출이 400으로 거절되므로 필수였음.
- 기존 언어 검수 수정: zh `AI摄像头`→`AI相机`(감시카메라 뉘앙스 제거), `弹力`→`弹性`(피부 탄력 표현), `医疗施术`→`医疗项目`(한국식 한자어 제거), 반각/전각 쉼표 통일, `留下预约咨询`→`填写预约咨询` / en `18M`+`Megapixels` 중복 → `18`+`Megapixels` / ja `4灯光源`→`4種の光源`.
